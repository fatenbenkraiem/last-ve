...
export class AdminresComponent implements OnInit {
  ...
  createActualite(){
    ...
    this.actualiteRes.image=this.myForm.value.image.substring(11).replace("/","");
   //console.log(this.actualiteRes.image)
  this.ressourcesService.createActualite(this.actualiteRes,id).subscribe(data=>{
      console.log(data)
      this.toastr.success({detail:"Message succès!",summary:"Ressource Ajoutée avec succès!",duration:4000})
      this.getActualitesadmin()
      this.redirectToList()
    }, error=>{
      console.log(error);
 ...
 var id= Object.values(act)[0]
    this.ressourcesService.deleteActualite(id).subscribe(data => {
      this.toastr.success({detail:"Message succès!",summary:"Ressource Supprimée avec succès!",duration:8000})
  ...
  updateActualiteRes(){
    ...
    this.ressourcesService.updateActualite(this.actualiteRes,this.actualiteRes.id).subscribe(data=>{
      this.getActualitesadmin()
      this.redirectToList()
      this.toastr.success({detail:"Message succès!",summary:"Ressource Modifiée avec succès!",duration:9000})
