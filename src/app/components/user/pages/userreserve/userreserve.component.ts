 ...
 id!:number
  ...
  getReservations() {
   this.reservation = new Reservation();
    this.reservationService.findReservationByIdUser(this.id).subscribe(data => {
      if (data != null) {
  ...
  createReservation() {
    ...
    this.reservationService.createReservation(this.reservation,this.idresorce,iduser).subscribe(data => {
      console.log(data)
      this.toastr.success({ detail: "Message succès!", summary:"Réservation Effectuée avec succès!", duration: 4000 })
      this.getReservations()
      this.redirectToList()
  ...
  updateReservation() {
   ...
   this.reservationService.updateReservations(this.reservation.id, this.reservation,this.reservation.ressources_id).subscribe(data => {
      this.toastr.success({ detail: "Message succès!", summary: "Réservation Modifier avec succès!", duration: 4000 })
      this.getReservations()
      this.redirectToList()
    }, error => {
      this.toastr.error({ detail: "Erreur", summary: "Serveur ne répond pas!" })
  ...
  deleteReservation(Reservation: Reservation) {
   ...
   // if this.reservation.etat = "disable"
    this.reservationService.deleteReservation(id).subscribe(data => {
      this.toastr.success({ detail: "Message succès!", summary: "Réservation  Annulée avec succès!", duration: 4000 })
    //  console.log(data);
      this.getReservations();
      this.redirectToList()
